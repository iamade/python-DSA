...
Here's a step-by-step explanation of the code block:
    ...
    fruit_list2 is assigned the reference of fruit_list1, so both lists now point to the same memory location. 
    Any changes made to either of them will be reflected in the other.

    fruit_list3 is assigned a slice of fruit_list1 that includes all elements. 
    This creates a new, shallow copy of the list,
    so changes made to fruit_list3 won't affect fruit_list1 or fruit_list2.

    The first element of fruit_list2 is changed to 'Guava'. 
    Since fruit_list1 and fruit_list2 refer to the same list, 
    the first element of fruit_list1 is also changed to 'Guava'.
